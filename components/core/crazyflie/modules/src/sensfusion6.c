...
static void sensfusion6UpdateQImpl(float gx, float gy, float gz,
...
{
  sensfusion6UpdateQWithMag(gx, gy, gz, ax, ay, az, 0.0f, 0.0f, 0.0f, dt);
}

void sensfusion6UpdateQWithMag(float gx, float gy, float gz,
                               float ax, float ay, float az,
                               float mx, float my, float mz,
                               float dt)
{
                                   ...
                                   float dt)
{
  ...
  if (!((mx == 0.0f) && (my == 0.0f) && (mz == 0.0f))) {
    ...
    float pitchSin = 2.0f * (qw * qy - qz * qx);
    if (pitchSin > 1.0f) {
      pitchSin = 1.0f;
    } else if (pitchSin < -1.0f) {
      pitchSin = -1.0f;
    }
    float pitch = asinf(pitchSin);
    ...
    float yawFromMag = atan2f(-magY, magX);
    float yawFromQuat = atan2f(2.0f * (qw * qz + qx * qy),
                               qw * qw + qx * qx - qy * qy - qz * qz);

    gz += wrapAngleRad(yawFromMag - yawFromQuat) * 2.0f;
  }
